...
- 本地 C ABI 必须由与当前 TUI 兼容的 core 源码构建，并通过显式
  `--runtime-library` 或独立链接加载；记录实际库路径、core SHA 和协议版本，不复用
  其他任务的库、虚拟环境或构建产物。优先使用 `scripts/build_runtime_local.py` 保持
  core 锁文件；共享 core 锁文件与构建目录的操作须串行或隔离。
