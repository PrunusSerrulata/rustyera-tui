...
测试集，不得重跑全量。所有端到端和长流程测试必须每 5 秒报告全部可观察界面元素与
runtime 状态；如果流程涉及 HTML 客户端，还必须包含全部 HTML 元素。忽略时间戳等报告
元数据后，连续两次内容相同必须立即按卡死失败退出。整个任务的测试流程从首条测试命令
开始共享 60 分钟墙钟预算；超时立即停止，并报告命令、用例/阶段、最后状态、已用时间及未验证项。
...
- 先运行与改动直接相关的最小 pytest，再按顺序运行一次完整 TUI pytest 和 Ruff：
