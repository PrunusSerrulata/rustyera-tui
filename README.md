...
成功编译的项目会生成仅供同一源码目录快速启动的内部缓存：

```text
.rustyera/cache/compiled-project.reracache
```

冷启动和“重启”使用持久化 stat/hash 源文件索引，因此不会重新读取未修改文件；精确
命中内部缓存时，也不必重新把源码 payload 传入 runtime；图片、音频等资源不会写入缓存，
而是继续由前端从源码目录读取。该缓存不是可分发的项目文件。运行中可重新加载全部脚本、
指定文件夹或单个脚本；文件夹和单脚本重载只提交所选范围，未选择的磁盘改动会保留到之后
选择对应范围或执行全量重载时。“返回标题”和 VM snapshot restore 复用已经加载的项目。

项目文件编码在短暂延迟后于后台启动，避免阻塞标题和第一天启动路径；完成后由前端原子
写入。

“文件”菜单中的“导出全量项目文件”会在用户主动选择目标后读取并打包全部资源，导出期间
显示进度并暂停游戏交互；后台内部缓存生成不会暂停交互。菜单也可导出或恢复 VM 快照。
“设置”菜单编辑 runtime
提供的当前项目配置：从源码目录启动时，可保存的设置会事务式写回项目配置；从
`.reraproj` 启动时，配置以紧凑的追加事务写入原项目文件，不需要重新生成完整文件。
